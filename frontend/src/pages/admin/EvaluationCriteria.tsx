...
export default function EvaluationCriteria() {
  ...
  const fetchEvaluationCriteria = async () => {
    try {
      ...
      const response = await fetch(`${import.meta.env.VITE_API_URL || 'http://localhost:5000'}/api/recruiter/evaluation-criteria`, {
        headers: {
  ...
  const handleResetToDefaults = async () => {
    ...
    try {
      const token = localStorage.getItem('recruiterToken');
      const response = await fetch(`${import.meta.env.VITE_API_URL || 'http://localhost:5000'}/api/recruiter/evaluation-criteria/reset`, {
        method: 'POST',
        headers: {
          'Authorization': `Bearer ${token}`
        }
